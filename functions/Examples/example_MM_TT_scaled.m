function [x,y,u,sig,sys_mats] = example_MM_TT_scaled(N,T)
...
for ii = 1:length(A)
    ...
    D{ii} = inv(T)*D{ii};
    H{ii} = inv(T)*H{ii};
    if(log(det(G{ii}*Q*G{ii}'))+log(det(H{ii}*R*H{ii}'))<0)
        disp('Error, beta_q+beta_r<0')
        return
    end
end
...
x0 = inv(T).*x0;
% x0 = zeros(4,1);
P0 = diag([(inv(T)*1)^2,(inv(T)*0.2)^2,(inv(T)*1)^2,(inv(T)*0.2)^2]);
