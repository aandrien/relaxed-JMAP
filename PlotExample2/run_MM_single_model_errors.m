function [rmsKF,rmsRDP_PD,rmsEM,percRDP_PD,percEM,t_KF,t_PD,t_EM] = run_MM_single_model_errors(N,T,e)
%% Main code
...
rmsKF = rms(eKF,2);
rmsRDP_PD = rms(eRDP_PD,2);
rmsEM = rms(eEM,2);

percRDP_PD = sum(Pset_PD(:,1) ==sig)/size(sig,1);
percEM = sum(sig_EM==sig)/size(sig,1);
